...
import paho.mqtt.client as mqtt # sudo pip install paho-mqtt works on windows and Raspberry Pi
import select
import re
import collections
import random
...
class MQTT_ReubenPython2and3Class(Frame): #Subclass the Tkinter Frame
    ...
    def __init__(self, setup_dict):
        ...
        print("MQTT_SERVER = " + str(self.MQTT_SERVER))
        ##########################################
        ...
        print("MQTT_USERNAME = " + str(self.MQTT_USERNAME))
        ##########################################
        ...
        print("MQTT_PASSWORD = " + str(self.MQTT_PASSWORD))
        ##########################################
        ...
        print("MQTT_PORT = " + str(self.MQTT_PORT))
        ##########################################
        ...
        print("MQTT_KEEPALIVE = " + str(self.MQTT_KEEPALIVE))
        ##########################################
        ...
        print("MQTT_Rx_topic_list = " + str(self.MQTT_Rx_topic_list))
        ##########################################
        ...
        print("MQTT_Rx_QOS_list = " + str(self.MQTT_Rx_QOS_list))
        ##########################################
        ...
        if "MQTT_Tx_topic_list" in setup_dict:
            ...
            for element in self.MQTT_Tx_topic_list:
                if element in RedundancyCheckList:
                    print(
                        "MQTT_ReubenPython2and3Class __init__ error: 'MQTT_Tx_topic_list' must not contain duplicate values.")
                    return
                else:
                    if type(element) != str:
                        print(
                            "MQTT_ReubenPython2and3Class __init__ error: 'MQTT_Tx_topic_list' must contain all strings.")
                        return
                    else:
                        RedundancyCheckList.append(element)
            ##############
        ...
        print("MQTT_Tx_topic_list = " + str(self.MQTT_Tx_topic_list))
        ##########################################
        ...
        print("MQTT_Tx_QOS_list = " + str(self.MQTT_Tx_QOS_list))
        ##########################################
        ...
        ##########################################
        if "RxThread_TimeToSleepEachLoop" in setup_dict:
            if self.my_platform == "pi": #Must have at least 0.001 sleep in each loop, or the RaspPi will seize-up!
                self.RxThread_TimeToSleepEachLoop = self.PassThroughFloatValuesInRange_ExitProgramOtherwise("RxThread_TimeToSleepEachLoop", setup_dict["RxThread_TimeToSleepEachLoop"], 0.001, 100000)
            else:
                self.RxThread_TimeToSleepEachLoop = self.PassThroughFloatValuesInRange_ExitProgramOtherwise("RxThread_TimeToSleepEachLoop", setup_dict["RxThread_TimeToSleepEachLoop"], 0.000, 100000)

        else:
            self.RxThread_TimeToSleepEachLoop = 0.002

        print("RxThread_TimeToSleepEachLoop: " + str(self.RxThread_TimeToSleepEachLoop))
        ##########################################
        ...
        print("MQTT_RxMessage_Queue_MaxSize: " + str(self.MQTT_RxMessage_Queue_MaxSize))
        ##########################################
        ...
        print("MQTT_TxMessage_Queue_MaxSize: " + str(self.MQTT_TxMessage_Queue_MaxSize))
        ##########################################
        ...
        ##########################################
        #self.ListeningThread_ThreadingObject = threading.Thread(target=self.ListeningThread, args=())
        #self.ListeningThread_ThreadingObject.start()
        ##########################################
    ...
    def GetMostRecentDataDict(self):
        ...
        if self.MQTT_RxMessage_Queue.qsize() > 0:
            ...
            return RxMessageToReturn_LocalCopy
        else:
            return dict()
    ##########################################################################################################
    ##########################################################################################################
    ...
    def RxThread(self):
        ...
        print("Started the RxThread thread for MQTT_ReubenPython2and3Class object.")
        self.RxThread_still_running_flag = 1
        ...
        while self.EXIT_PROGRAM_FLAG == 0:
            ...
            try:
                ...
                ########################################################
                ########################################################
                self.UpdateFrequencyCalculation_CalculatedFromRxThread()  # ONLY UPDATE IF WE HAD NEW DATA

                if self.RxThread_TimeToSleepEachLoop > 0.0:
                    time.sleep(self.RxThread_TimeToSleepEachLoop)
                ########################################################
                ########################################################
                ...
                exceptions = sys.exc_info()[0]
                self.MyPrint_WithoutLogFile("MQTT_ReubenPython2and3Class RxThread: Exceptions: %s" % exceptions)
        ...
        print("Finished the RxThread for MQTT_ReubenPython2and3Class object.")
        self.RxThread_still_running_flag = 0
